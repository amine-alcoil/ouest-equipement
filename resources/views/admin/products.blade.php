            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div class="md:col-span-2">
                    <div class="flex items-center justify-between mb-2">
                        <label class="block text-sm font-medium text-white/80">Fiche technique (PDF)</label>
                        <div id="editCurrentPdfContainer" class="flex items-center gap-2"></div>
                    </div>
                    <div class="relative group">
    ...
    const bindRowActions = () => {
        ...
        document.querySelectorAll('[data-edit-id]').forEach(link => {
            link.addEventListener('click', async (e) => {
                ...
                const currentPdfContainer = document.getElementById('editCurrentPdfContainer');
                ...
                if (currentPdfContainer) {
                    currentPdfContainer.innerHTML = ''; // Clear previous
                    ...
                    if (data.product.pdf && typeof data.product.pdf === 'string' && data.product.pdf.trim() !== '') {
                        ...
                        currentPdfContainer.innerHTML = `
                            <a href="${data.product.pdf}" target="_blank" class="flex items-center gap-1 text-emerald-400 hover:text-emerald-300 transition-colors bg-emerald-500/10 px-2 py-1 rounded border border-emerald-500/20" title="${pdfName}">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                                <span class="max-w-[150px] truncate">${pdfName}</span>
                            </a>
                    ...
                    if (currentPdfContainer) {
                        ...
                        if(newDelBtn) {
                            newDelBtn.onclick = async (e) => {
                                ...
                                if(confirmed) {
                                    currentPdfContainer.innerHTML = `
                                        <div class="flex items-center justify-between p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 w-full group/del">
                                            <div class="flex items-center gap-3 truncate">
                                                <div class="p-2 rounded-lg bg-red-500/20">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                                </div>
                                                <div class="flex flex-col truncate">
                                                    <span class="text-[10px] font-bold uppercase tracking-wider opacity-60">À supprimer</span>
                                                    <span class="text-sm font-medium line-through truncate opacity-50">${name}</span>
                                                </div>
                                            </div>
                                            <input type="hidden" name="delete_pdf" value="1">
                                            <button type="button" class="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-[10px] font-bold uppercase tracking-widest transition-all border border-white/5" onclick="window.revertPdfDeletion('${url}', '${name}')">Annuler</button>
                                        </div>
                                    `;
                                }
                            };
